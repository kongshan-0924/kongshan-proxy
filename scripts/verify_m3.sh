...
test_bundles=(${(f)"$(kongshan_all_test_bundles "$project_dir")"})
(( ${#test_bundles[@]} >= 1 )) || {
    print -u2 -- "找不到 XCTest bundle（两种布局都没命中）"
    exit 1
}
for test_bundle in $test_bundles; do
    print -- "运行测试包：${test_bundle:t}"
...
"$core_binary" version | grep -q 'sing-box version 1.13.21'
