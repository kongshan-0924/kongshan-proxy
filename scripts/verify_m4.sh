...
"$core_binary" version | grep -q 'sing-box version 1.13.21' || fail "unexpected sing-box version"

if [[ ${KONGSHAN_VERIFY_SKIP_TESTS:-0} != 1 ]]; then
    source "$project_dir/scripts/test_bundle_lib.zsh"
    run_gate_test() {
        local test_id=$1 bundle
        bundle=$(kongshan_bundle_for_test "$project_dir" "$test_id") \
            || fail "找不到承载 $test_id 的 XCTest bundle"
        xcrun xctest -XCTest "$test_id" "$bundle"
    }
    run_gate_test KongshanCoreTests.DNSConfigTests/testSystemAndTUNDefaultAndCustomDNSPassBundledCoreCheck
    run_gate_test KongshanCoreTests.ClashStreamingTests/testConsumerCancellationClosesUnderlyingDataStream
    run_gate_test KongshanCoreTests.CrashRestartTests
    run_gate_test KongshanAppTests.AppStateTests/testDashboardMonitoringKeepsSixtyPointsAndIsIdempotent
    run_gate_test KongshanAppTests.AppStateTests/testLogMonitoringKeepsTwoThousandEntriesAndIsIdempotent
    run_gate_test KongshanAppTests.AppStateTests/testInitializeOnlyReadsLoginItemStatusWithoutRegistering
fi
