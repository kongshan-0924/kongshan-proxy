...
**发布只做加法。** 不再清理旧 Release、旧标签、`dist` 里的历史 DMG，或仓库中的任何旧内容。
用户需要可追溯的历史版本与回滚点。具体：

- **不执行** `gh release delete`、`git tag -d`、删除远端标签；
- `make_dmg.sh` **不再**删除 `dist` 里的历史 DMG（只清可运行 App 副本，它会被
  Launch Services 记住并导致误启动第二个实例）；
- 本文件下方及 `PROGRESS.md`/`SESSION_LOG.md` 中「只保留最新 Release」「旧标签已清理」
  等表述是**历史记录**，如实反映当时的做法，不改写；但**不再作为今后的做法**。
