...
## 当前状态：v0.1.71 修复端口漂移，已通过完整 M4（2026-08-08）

- v0.1.70 已安全安装并完成 UI、系统代理、全量测速、配置往返、helper 与 TUN 真机验收；配置“奶昔”及
  用户文件逐项保留。TUN 关闭后 root 内核退出、DNS 恢复 `192.168.2.1`、直连 Apple HTTPS 200，
  当前安装版以系统代理模式运行，端口为 `55996`，网络正常。
- 真机发现 TUN 切回系统代理时持久 mixed 端口从 `56758` 漂移到 `55996`。helper 的 stopTun 已确认会
  等待子进程退出，并非提前回成功；本机 `sysctl` 进一步确认旧分配范围 `49152...65535` 与 macOS
  临时源端口池完全重叠，旧监听释放后可能被任意客户端短暂抢占。这是根因，不是单纯等待不够。
- v0.1.71 将 App/helper 共用监听范围改为 `20000...49151`，随机试绑空闲端口，避开系统临时池；升级前
  的旧高位端口会一次性迁移。新增回归测试先稳定失败 32/32 次，修复后端口、白名单、TUN 组合、DNS
  分流和配置生成定向测试全部通过。
- 新鲜 M4：432 项执行、1 项按预期跳过、0 失败；Release/arm64/deep+strict/sing-box 1.13.14、
  helper 白名单和恢复残留检查通过。五次空闲 CPU 1.6/0.0/0.0/0.0/0.0%，平均 0.320%，最大 RSS
  114,016 KB；候选为 v0.1.71/build 171，App CDHash `dfbc3949cce126a55280aa85f2fe55a036bb3c2c`。
- 当前边界：v0.1.71 尚未提交、生成提交绑定的最终 DMG、安装或发布。替换必须正常退出 v0.1.70，
  确认系统代理/DNS/recovery/直连恢复并备份配置后原子安装；新 cdhash 需要重新安装 helper 后再做
  TUN → 系统代理端口稳定性终验。
