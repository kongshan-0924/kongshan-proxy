...
## 当前状态：v0.1.100 已构建待手动安装；v0.1.99 在本机运行中；**均未发布 GitHub**（2026-09-04）

- **v0.1.100**：修复「系统代理 + TUN 都开着时切配置必然失败并回滚」——`enable()` 幂等短路 +
  瞬时错误重试预算 0.6s→3s + 给系统 DNS 补上同一套重试；另并入连接页 `/connections` 重复订阅的合流
  （同一份 payload 不再每秒解析两遍）。按用户要求只构建、不替换本地安装。

- **01:10 续做**：仪表盘 Observation 收窄（仪表盘可见时 CPU 中位 6.3% vs 其它页 1.5% 的结构性原因）；
  CPU 异常持续时自动 `sample` 自身保存调用栈到 `samples/`；代理/配置页去幽灵条纹、规则页分应用区
  两行布局与规则浏览器标题、指标卡数值紧贴说明。详见 SESSION_LOG 2026-09-04 01:10 条。

- **本轮修的问题**：停止系统代理 / TUN 时，此刻不在网络服务列表里的服务被 `restoreFromDisk` 静默跳过、
  快照也被删掉——真机 2026-09-03「Shadowrocket」服务回到列表后三项代理仍指向已停的 `127.0.0.1:36815`，
  用户关了代理 Codex 等直连请求全 404。系统 DNS 侧同一模式；`AppState` 三处回滚路径 `try?` 吞还原失败。
- **修法**：待还原保留（快照不删、服务回来自动复位、期间不拒绝再次接管）＋ 写回后读回核对 ＋
  不依赖快照的残留清扫（启动 / 换网，只对接管过的安装生效，记事件）＋ 回滚失败写进错误与事件；
  另把反向解析 / Bonjour 查询留在本地解析器。
- **验证**：全量 588 / 2 跳过 / 0 失败；新增 13 条用例、改写 3 条旧用例。**未跑 `prepare`、未构建、未提交**，
  等用户决定是否发布（`scripts/release.sh prepare → publish`；安装由用户决定）。
- **残留已清**：Shadowrocket 服务的三项代理已手动关掉并读回核对。
- 详情：`docs/progress/SESSION_LOG.md` 2026-09-03 23:40 与 2026-09-04 00:35 两条；用户可见变化见 `CHANGELOG.md` v0.1.99。

**v0.1.83 → v0.1.98 补记**（此文档此前停在 v0.1.82；逐版要点在 `CHANGELOG.md`，逐轮记录在 `SESSION_LOG.md`）：
v0.1.90 界面刷新 → v0.1.91 开机后自动恢复接管 → v0.1.92 直连域名本地解析 → v0.1.93 告警持久化到
`diagnostics.ndjson` 且不被"全部清除"抹掉 → v0.1.94 故障检测不依赖日志页可见 → v0.1.95 每分钟运行指标
流水 `metrics.ndjson` → v0.1.96 原生 macOS 形态 UI（`docs/design/NATIVE_UI.md`）→ v0.1.97 局域网共享 +
三处界面反馈 → v0.1.98 局域网共享独立成「共享」页（独立监听、独立端口、白名单、已接入设备用量）。
发布保留策略：GitHub Release / 标签只增不删，本地 `dist` 只留最新 DMG。

## 历史状态：v0.1.82 源码完成（DNS 引导 + 旧 Release 清理 + 阈值验证）（2026-08-23）
...
### 发布保留策略（2026-08-23 用户明确要求，优先于本文件下方所有历史记录）

**远端保留全部历史，本地只留最新。** 版本历史的载体是 GitHub，不是本机磁盘。

- **远端（GitHub）**：保留全部 Release 与标签。**不执行** `gh release delete`、
  `git tag -d`、删除远端标签。用户需要可追溯的历史版本与回滚点。
- **本地 `dist/`**：只保留最新 DMG，`make_dmg.sh` 照旧清理旧的。本地 DMG 是一次性产物，
  任何历史版本都能从对应标签重新构建，或直接从 Releases 下载。
- **仓库内容**（代码、文档、历史记录）：不删除。
- `dist/kongshan.app` 可运行副本仍要清掉——Launch Services 会记住它并导致误启动第二个实例，
  这是早期踩过的坑，与版本历史无关。
- 本文件下方及 `PROGRESS.md`/`SESSION_LOG.md` 中「只保留最新 Release」「旧标签已清理」
  等表述是**历史记录**，如实反映当时的做法，不改写；但**远端清理部分不再作为今后的做法**。

此前已被删除且无法恢复的 Release 资产：v0.1.73/77/79/80（元数据与提交号存档于
SESSION_LOG「2026-08-23 02:55」条，标签可据此重建，DMG 不可恢复）。
