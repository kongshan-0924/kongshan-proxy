...
## 当前状态：v0.1.70 发布候选已通过完整 M4（2026-08-08）
...
- 新鲜验证：完整 M4 执行 431 项、1 项按预期跳过、0 失败；Release/arm64/deep+strict/sing-box
  1.13.14/规则集配置均通过。五次空闲 CPU 0.4/0.0/0.3/0.0/0.4%，平均 0.220%，最大 RSS
  123,360 KB；无 TCP socket、子进程、FIFO 或 recovery 残留。
- 当前边界：工作区 `VERSION` 已为 0.1.70/build 170；正式安装版仍是 v0.1.68，正以系统代理模式运行。
  尚未退出旧版、生成最终 v0.1.70 DMG、提交、安装或发布；这些必须按发布门禁顺序完成。
