...
| 页 | 工具栏 | 内容 |
| --- | --- | --- |
| 仪表盘 | — | 状态 `GroupBox`（状态点 + 文字 + 节点 **+ 当前配置** + 实时速率；第二行出站模式/接管开关）；指标网格**四张卡**（出口 IP / 活跃连接 / 内存（内核 / 空山）/ 运行时长，列数只取 4 的因数）；流量 `GroupBox` |
| 配置 | 刷新全部 · 自建节点 | 导入栏（圆角文本框 + 按钮）；`List(.inset)` **不用交替行底色**（条目少时条纹会铺满空白画出幽灵行，v0.1.99 去掉），系统符号替代渐变图标块 |
| 代理 | 出站模式 · 排序 · 测速全部 · 测速并选最快；`.searchable` | 左 `List(.sidebar)` 策略；右 **`List` 选择即切换节点**（不用交替行底色），一行：选中标记（未选中行留空，不放机架符号）· 国旗 · 名称 · 协议 · 倍率 · 延迟 |
| 规则 | `.searchable`（搜规则） | 上半 `Form(.grouped)`：规则开关分区 + 四个可折叠分区（`Section(isExpanded:)`）；下半规则浏览 `List` |
| 出口分析 | 重新自测 · 刷新出口 · 复制报告 | 三个 `GroupBox`：出口 IP / 站点可达性（含结论句与逐站结果）/ DNS 泄漏（含解析器明细） |
