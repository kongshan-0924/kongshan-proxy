...
final class HelperTrustEvaluationTests: XCTestCase {
    ...
    /// 新 schema 必须同时具备 bundle 路径、两个 cdhash 与 uid（v4 起），
    /// 否则 helper 拒绝加载整份 trust。
...
final class HelperTrustConfigDecodingTests: XCTestCase {
    ...
    func testOnlyCompleteCurrentTrustConfigIsAcceptedByHelper() {
        ...
        // v4 起 uid 也是必填项：缺了就是旧配置，必须走重装而不是"兼容"地放行。
        var withoutUID = current
        withoutUID.pinnedUID = nil
        XCTAssertFalse(withoutUID.isCurrent)
