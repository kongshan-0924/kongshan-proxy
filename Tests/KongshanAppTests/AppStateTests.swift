...
final class AppStateTests: XCTestCase {
    ...
    func testOfflineSSHProxyTargetPersistsWithoutLeavingActiveProxyCommand() async throws {
        ...
        let added = await state.upsertSSHProxyTarget(address: "203.0.113.10", port: 2_222)
        XCTAssertTrue(added)

        XCTAssertEqual(state.sshProxyTargets, [SSHProxyTarget(address: "203.0.113.10", port: 2_222)])
    ...
    func testRunningSSHProxyTargetInstallsAfterReloadAndRemovesOnStop() async throws {
        ...
        let added = await fixture.state.upsertSSHProxyTarget(address: "203.0.113.10", port: 2_222)
        XCTAssertTrue(added)
        var calls = await sshManager.values()
        XCTAssertEqual(calls.last?.targets, [SSHProxyTarget(address: "203.0.113.10", port: 2_222)])
    ...
    func testFailureToRestoreOldCoreRestoresSystemProxyAndMarksFailed() async throws {
        let sshManager = RecordingSSHProxyConfigManager()
        let sshTarget = SSHProxyTarget(address: "203.0.113.10", port: 2_222)
        let fixture = try await makeRunningFixture(
            healthFailures: [2, 3],
    ...
    /// 只用 example 域名的订阅夹具：两份 rule-providers（classical 含进程条件、domain 通配）、
    /// GEOIP 与 MATCH，结构仿照真实机场订阅。
    private nonisolated static let ruleSetSubscriptionYAML = """
    proxies:
      - {name: rs-node, type: ss, server: 127.0.0.1, port: 9, cipher: aes-128-gcm, password: secret}
    proxy-groups:
      - {name: 节点选择, type: select, proxies: [rs-node, DIRECT]}
      - {name: AI, type: select, proxies: [节点选择, rs-node]}
      - {name: 全球直连, type: select, proxies: [DIRECT, 节点选择]}
      - {name: 漏网之鱼, type: select, proxies: [节点选择, DIRECT]}
    rule-providers:
      ai: {type: http, behavior: classical, format: yaml, url: "https://rules.example.com/ai.yaml", interval: 43200}
      direct: {type: http, behavior: domain, format: yaml, url: "https://rules.example.com/direct.yaml", interval: 43200}
    rules:
      - RULE-SET,ai,AI
    ...
    private nonisolated static let ruleSetBodies = [
        "/ai.yaml": "payload:\n  - DOMAIN-SUFFIX,ai.example\n  - PROCESS-NAME,AIHelper\n",
        "/direct.yaml": "payload:\n  - '+.direct.example'\n"
    ]

    private func ruleSetFixture(
        initialMode: ProxyMode, server: RuleSetFakeServer, subscriptionOnline: OnlineSwitch? = nil
    ) async throws -> (fixture: ModeFixture, sourceID: UUID) {
        let binary = singBoxURL
        let fixture = try await makeModeFixture(
            initialMode: initialMode,
            subscriptionLoader: { _ in
                if let subscriptionOnline, !subscriptionOnline.isOn { throw URLError(.notConnectedToInternet) }
                return HTTPDownload(data: Data(Self.ruleSetSubscriptionYAML.utf8), statusCode: 200)
            },
            subscriptionRuleSetService: { storage in
                SubscriptionRuleSetService(
                    storage: storage,
                    downloader: { url, _ in try await server.download(url) },
                    compiler: SubscriptionRuleSetService.coreCompiler(binaryURL: binary)
                )
            }
        )
        await fixture.state.importSubscription(url: URL(string: "https://example.com/rs.yaml")!, name: "rs")
        return (fixture, try XCTUnwrap(fixture.state.subscriptions.first?.id))
    }

    private func waitLonger(_ condition: () -> Bool, file: StaticString = #filePath, line: UInt = #line) async throws {
        for _ in 0..<500 {
            if condition() { return }
    ...
    /// 核心体验：规则集还没下载时，配置里是同路径的空占位，内核照常启动；下载完成后原地替换，
    /// **不重启内核**。MATCH 成为兜底出口。
    func testSubscriptionRuleSetsStartAsPlaceholdersAndLoadWithoutRestart() async throws {
        let server = RuleSetFakeServer(bodies: Self.ruleSetBodies)
        server.hold()
        let (fixture, sourceID) = try await ruleSetFixture(initialMode: .tun, server: server)
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        XCTAssertEqual(fixture.state.discoveredRuleProviders[sourceID]?.map(\.name), ["ai", "direct"])
        XCTAssertEqual(fixture.state.discoveredMatchTargets[sourceID], "漏网之鱼")

        await fixture.state.setActiveConfig(sourceID)
        XCTAssertEqual(fixture.state.status, .on)
        XCTAssertTrue(fixture.state.isDownloadingActiveRuleSets)
        let configs = await fixture.privileged.startedConfigs()
        let config = try XCTUnwrap(JSONSerialization.jsonObject(with: try XCTUnwrap(configs.last)) as? [String: Any])
        let route = try XCTUnwrap(config["route"] as? [String: Any])
        XCTAssertEqual(route["final"] as? String, "漏网之鱼", "订阅的 MATCH 是兜底出口")
        let declared = try XCTUnwrap(route["rule_set"] as? [[String: Any]])
        var paths: [String: String] = [:]
        for definition in declared {
            if let tag = definition["tag"] as? String, let path = definition["path"] as? String { paths[tag] = path }
        }
        for tag in ["sub-ai", "sub-ai-dns", "sub-direct", "sub-direct-dns"] {
            XCTAssertNotNil(paths[tag], "\(tag) 要写进配置（未下载时是占位）")
        }
        let aiFile = URL(fileURLWithPath: try XCTUnwrap(paths["sub-ai"]))
        let placeholder = try Data(contentsOf: aiFile)
        XCTAssertLessThan(placeholder.count, 64, "还没下载时是空规则集占位")

        server.release()
        try await waitLonger { fixture.state.activeSubscriptionRuleSets.count == 2 }
        XCTAssertFalse(fixture.state.isDownloadingActiveRuleSets)
        XCTAssertEqual(fixture.state.activeSubscriptionRuleSets["ai"]?.entryCount, 2)
        let after = await fixture.privileged.startedConfigs()
        XCTAssertEqual(after.count, configs.count, "下载完成不重启内核：内核自己重载被原子替换的文件")
        XCTAssertGreaterThan(try Data(contentsOf: aiFile).count, placeholder.count, "占位已被原地替换为正式内容")
        await fixture.state.stop()
    }

    /// 命中测试按已下载的规则集内容判定（含单独成条的进程条件），没命中的走 MATCH。
    func testRouteTestJudgesDownloadedRuleSetsAndMatchTarget() async throws {
        let server = RuleSetFakeServer(bodies: Self.ruleSetBodies)
        let (fixture, sourceID) = try await ruleSetFixture(initialMode: .systemProxy, server: server)
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        try await waitLonger { fixture.state.subscriptionRuleSetCache[sourceID]?.count == 2 }
        await fixture.state.setActiveConfig(sourceID)

        let ai = await fixture.state.testRoute(domain: "chat.ai.example", ip: nil, processName: nil)
        XCTAssertEqual(ai.source, .subscription)
        XCTAssertEqual(ai.target, "AI")
        XCTAssertTrue(ai.matchedValue.contains("规则集 ai"), ai.matchedValue)
        let process = await fixture.state.testRoute(domain: nil, ip: nil, processName: "AIHelper")
        XCTAssertEqual(process.target, "AI")
        let direct = await fixture.state.testRoute(domain: "www.direct.example", ip: nil, processName: nil)
        XCTAssertEqual(direct.target, "全球直连")
        let other = await fixture.state.testRoute(domain: "unlisted.example.org", ip: nil, processName: nil)
        XCTAssertEqual(other.source, .final)
        XCTAssertEqual(other.target, "漏网之鱼")
        XCTAssertEqual(other.matchedValue, "MATCH")
        await fixture.state.stop()
    }

    /// 删除订阅时它的规则集目录一并清掉；重启后已下载的规则集从磁盘读回、不再联网。
    func testRuleSetsSurviveRelaunchAndAreRemovedWithSubscription() async throws {
        let server = RuleSetFakeServer(bodies: Self.ruleSetBodies)
        let (fixture, sourceID) = try await ruleSetFixture(initialMode: .systemProxy, server: server)
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        try await waitLonger { fixture.state.subscriptionRuleSetCache[sourceID]?.count == 2 }
        await fixture.state.setActiveConfig(sourceID)
        await fixture.state.stop()
        let requestsBeforeRelaunch = server.requestCount

        let storage = Storage(rootDirectory: fixture.root)
        let binary = singBoxURL
        let reloaded = AppState(
            storage: storage,
            subscriptionRuleSetService: SubscriptionRuleSetService(
                storage: storage,
                downloader: { url, _ in try await server.download(url) },
                compiler: SubscriptionRuleSetService.coreCompiler(binaryURL: binary)
            ),
            ...
            singBoxProcess: SingBoxProcess(binaryURL: URL(fileURLWithPath: "/usr/bin/false")),
            privilegedLauncher: FakePrivilegedLauncher(root: fixture.root),
            automaticallyInitialize: false
        )
        await reloaded.initialize()
        XCTAssertEqual(reloaded.activeConfigID, sourceID)
        XCTAssertEqual(reloaded.discoveredRuleProviders[sourceID]?.map(\.name), ["ai", "direct"])
        XCTAssertEqual(reloaded.activeMatchTarget, "漏网之鱼")
        XCTAssertEqual(reloaded.activeSubscriptionRuleSets.count, 2)
        XCTAssertFalse(reloaded.isDownloadingActiveRuleSets)
        XCTAssertEqual(server.requestCount, requestsBeforeRelaunch, "有缓存的启动不联网")

        let directory = fixture.root.appending(path: "rule-sets/subscription/\(sourceID.uuidString.lowercased())")
        XCTAssertTrue(FileManager.default.fileExists(atPath: directory.path))
        await reloaded.removeSubscription(id: sourceID)
        XCTAssertNil(reloaded.discoveredRuleProviders[sourceID])
        XCTAssertNil(reloaded.subscriptionRuleSetCache[sourceID])
        try await waitLonger { !FileManager.default.fileExists(atPath: directory.path) }
    }
    ...
    func testFailedRuleSetDownloadWarnsAndRecoversOnManualUpdate() async throws {
        ...
        XCTAssertEqual(fixture.state.activeSubscriptionRuleSets.count, 2)
        XCTAssertNil(fixture.state.errorMessage)
        await fixture.state.stop()
    }

    /// 订阅本身没刷下来（离线）时不连带下规则集——那只会多出一串同样原因的失败；
    /// 后来订阅与规则集都恢复了，两类旧的失败告警自动清掉，不留在日志里误导人。
    func testSubscriptionFallbackSkipsRuleSetsAndRecoveryClearsWarnings() async throws {
        let server = RuleSetFakeServer(bodies: Self.ruleSetBodies)
        server.fail("/ai.yaml")
        server.fail("/direct.yaml")
        let online = OnlineSwitch()
        let (fixture, sourceID) = try await ruleSetFixture(
            initialMode: .systemProxy, server: server, subscriptionOnline: online
        )
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        ...
        online.isOn = true
        server.recover("/ai.yaml")
        server.recover("/direct.yaml")
        await fixture.state.refreshSubscription(id: sourceID)
        try await waitLonger { fixture.state.subscriptionRuleSetCache[sourceID]?.count == 2 }
        XCTAssertFalse(
            fixture.state.warnings.contains { $0.hasPrefix("订阅「rs」") },
    ...
    /// 不发通知，5 分钟后再看（2026-09-19 真机：一夜攒下订阅离线失败与「通知未发送」）。
    func testScheduledRefreshWaitsQuietlyWhileOffline() async throws {
        let root = temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let storage = Storage(rootDirectory: root)
        let now = Date(timeIntervalSince1970: 20_000)
        ...
        let state = AppState(
            ...
            subscriptionService: SubscriptionService(storage: storage) { _ in
                ...
                return HTTPDownload(data: Data(updatedYAML.utf8), statusCode: 200)
            },
            systemProxyManager: SystemProxyManager(storage: storage) { _, _ in
                ProcessResult(exitCode: 0, stdout: "", stderr: "")
            },
        ...
        await state.setSubscriptionUpdateSettings(SubscriptionUpdateSettings(enabled: true, intervalHours: 1))
        try await waitUntil { state.nextSubscriptionUpdateAt == now.addingTimeInterval(5 * 60) }
        let loadCount = await loads.count
        let notificationCount = await notifications.count
        XCTAssertEqual(loadCount, 0, "没网时不该发起订阅下载")
        XCTAssertEqual(notificationCount, 0)
        XCTAssertTrue(state.warnings.isEmpty, "\(state.warnings)")
        await scheduler.cancel()
    }

    /// 局域网共享只用设置里的端口；被占用时**不悄悄换端口**（别的设备存的就是它），
    /// 而是停下并说明原因；换一个端口后恢复。
    func testLANSharingUsesConfiguredPortAndReportsConflictInsteadOfSwitching() async throws {
        let fixture = try await makeRunningFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        fixture.relay.setLANStartError(LocalTCPRelayError.portInUse(7890))
        await fixture.state.applyLANSharing(LANSharingSettings(enabled: true, port: 7890, allowedCIDRs: []))
        XCTAssertNil(fixture.state.lanSharingBoundPort)
        XCTAssertEqual(fixture.state.lanSharingFailure?.contains("端口 7890 已被其他程序占用"), true)
        XCTAssertFalse(fixture.state.warnings.contains { $0.contains("已改用") })

        fixture.relay.setLANStartError(nil)
        await fixture.state.applyLANSharing(LANSharingSettings(enabled: true, port: 7891, allowedCIDRs: []))
        XCTAssertEqual(fixture.state.lanSharingBoundPort, 7891)
        XCTAssertNil(fixture.state.lanSharingFailure)
        XCTAssertEqual(fixture.relay.lanPortsRequested, [7890, 7891])
        await fixture.state.stop()
    }
    ...
    private func makeModeFixture(
        ...
        subscriptionRuleSetService: ((Storage) -> SubscriptionRuleSetService)? = nil
        ...
        let state = AppState(
            ...
            subscriptionRuleSetService: subscriptionRuleSetService?(storage),
            systemProxyManager: SystemProxyManager(storage: storage, runner: network.run(arguments:timeout:)),
            systemDNSManager: SystemDNSManager(storage: storage, runner: network.run(arguments:timeout:)),
            singBoxProcess: core,
            processExitMonitor: processExitMonitor,
...
/// 订阅规则集的假下载源：按路径返回内容，可整体扣住（模拟慢网络）、可让某个路径失败。
private final class RuleSetFakeServer: @unchecked Sendable {
    private let lock = NSLock()
    ...
    func download(_ url: URL) async throws -> Data {
        while lock.withLock({ held }) {
            try await Task.sleep(for: .milliseconds(10))
        }
        return try lock.withLock {
            requests += 1
            if failing.contains(url.path) { throw URLError(.timedOut) }
            guard let body = bodies[url.path] else { throw URLError(.fileDoesNotExist) }
...
private final class FakeLocalTCPRelay: LocalTCPRelaying, @unchecked Sendable {
    ...
    private var lanStartError: Error?
    private var requestedLANPorts: [UInt16] = []

    func setLANStartError(_ error: Error?) { lock.withLock { lanStartError = error } }
    var lanPortsRequested: [UInt16] { lock.withLock { requestedLANPorts } }
    ...
    func startLANSharing(port: UInt16, policy: LANPeerPolicy) async throws -> UInt16 {
        try lock.withLock {
            requestedLANPorts.append(port)
            if let lanStartError { throw lanStartError }
...
extension AppStateTests {
    func testLANProbeFeedsSnapshotAndSystemProxyBypass() async throws {
        ...
        XCTAssertEqual(fixture.state.lanResolverSnapshot.servers, ["10.0.0.53"])
    ...
    func testNetworkChangeReprobesLANDNSAndReloadsTUN() async throws {
        let responses = LANResolverResponses([
            ...
            LANResolverSnapshot(servers: ["10.0.0.53"], searchDomains: ["new.corp.example"])
        ])
        let fixture = try await makeModeFixture(
            initialMode: .tun,
        ...
        XCTAssertEqual(lan["server"] as? String, "10.0.0.53")
