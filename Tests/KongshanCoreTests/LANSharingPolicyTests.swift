...
/// S3（2026-09-17 审计）：CGNAT 共享地址段**刻意**不算私网。
///
/// 这条守着一个容易被"顺手修掉"的决定：100.64.0.0/10 看起来像内网、
/// 手机热点场景也确实会落在里面，但它是运营商级 NAT 的共享段——同运营商的其他用户
/// 可能就在同一个 /10 内。局域网共享入口无鉴权，放行它等于开放代理。
extension LANSharingPolicyTests {
    func testCarrierGradeNATIsNotTreatedAsPrivate() {
        for address in ["100.64.0.1", "100.100.5.7", "100.127.255.254"] {
            let endpoint = NWEndpoint.hostPort(host: .init(address), port: 7890)
            XCTAssertFalse(
                LocalTCPRelay.isPrivatePeer(endpoint),
                "\(address) 属于 CGNAT 共享段，不是信任边界，不该被当成私网放行"
            )
        }
    }

    func testRFC1918RangesRemainPrivate() {
        for address in ["10.0.0.1", "172.16.0.1", "172.31.255.254", "192.168.1.1", "169.254.1.1"] {
            let endpoint = NWEndpoint.hostPort(host: .init(address), port: 7890)
            XCTAssertTrue(LocalTCPRelay.isPrivatePeer(endpoint), "\(address) 应被认作私网")
        }
    }
}
