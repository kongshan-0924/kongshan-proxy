...
final class RoutingConfigTests: XCTestCase {
    ...
    func testSSHProxyTargetCreatesPortScopedRuleAndMixedInboundForTun() throws {
        var settings = RoutingSettings.defaults
        settings.sshProxyTargets = [SSHProxyTarget(address: "203.0.113.10", port: 2_222)]
        ...
        XCTAssertEqual(rules[1]["ip_cidr"] as? [String], ["203.0.113.10/32"])
        XCTAssertEqual(rules[1]["port"] as? [Int], [2_222])
...
extension RoutingConfigTests {
    func testSubscriptionRulesParseAndEnterConfigWithUnresolvableTargetsDropped() async throws {
        ...
        // 2026-09-18 起 GEOIP 受支持（按原位置引用内置 geoip 规则集）；MATCH 是兜底出口不是规则，
        // 由 parseMatch 单独取出。详见 SubscriptionRuleSetParsingTests。
        XCTAssertEqual(
            SubscriptionRule.parse("GEOIP,CN,DIRECT"),
