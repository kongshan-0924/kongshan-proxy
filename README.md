...
启动时会先探测；代理运行期间如果物理网络身份确实变化（例如从 Wi-Fi 切到手机热点），也会重新探测 LAN DNS、重建配置并关闭旧连接。普通信号抖动和 IPv6 续租不会触发重载。
...
- **系统代理入口固定**：App 自己持有一个只监听 `127.0.0.1` 的轻量 TCP relay，公开端口写入 `settings.json` 跨启动复用；sing-box 每次启动使用独立的内部 mixed 端口。系统代理始终连接 relay，TUN 开关只切换 relay 的后端，不再让 root 与普通用户进程交接同一个监听端口。这样既保留 Chromium 系客户端缓存所需的稳定地址，也避免切换期间端口漂移和反复「正在重新连接」。relay 只转发原始 TCP 字节，不解析 HTTP/SOCKS，也不设置 `SO_REUSEPORT`。
