...
按机场自带的策略分流：在主组里挑一个出站节点（可先「测速全部」），需代理的策略会跟随它，也可为单个策略单独指定。延迟显示在节点卡片右上角，选中项有底色。切换节点后自动断开旧连接，出口 IP 立即生效。
...
- **网速统计整机网卡**（`en*` 物理接口），不是代理流量，所以代理没开也有读数。不把所有接口相加是有意的：TUN 开着时一份流量会在 `utun`（明文）和 `en0`（密文）各记一遍。
...
`verify_m4.sh` 运行全量测试，构建并校验 arm64 App / 签名 / 内核 / 规则集，再在隔离的临时数据目录启动无节点 App，检查 CPU、RSS、TCP socket、子进程和恢复残留。脚本不会设置真实系统代理、注册登录项、请求 TUN 授权或发送真实通知。
