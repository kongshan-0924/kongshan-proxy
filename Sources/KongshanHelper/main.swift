...
func extractClientIdentity(connfd: Int32) -> HelperClientIdentity? {
    ...
    // v4：从**同一个** audit_token 取对端 euid。
    //
    // 必须与签名信息同源：走 `LOCAL_PEERPID` → `getpwuid` 之类的裸 PID 往返有竞态
    //（PID 可被回收后复用），而 audit_token 是内核在建连那一刻快照下来的。
    //
    // `audit_token_to_euid()` 在 <bsm/libbsm.h> 里是宏（`((atoken).val[1])`），
    // C 宏不会被导入 Swift，所以直接读元组下标。libbsm 的字段次序是固定 ABI：
    // val[0]=auid val[1]=euid val[2]=egid val[3]=ruid val[4]=rgid val[5]=pid val[6]=asid val[7]=pidversion。
    // 取 euid 而非 ruid：euid 才是决定权限的那个，也是 XPC 的 `xpc_connection_get_euid` 的语义。
...
func adoptOrphanKernel() {
    ...
    if let clientPID = firstPID(withExecutablePath: clientPath) {
        ...
        // 同 checkClientLiveness：先还原 DNS 再停内核。
        // 这条路径是助手自己重启后认领残留内核，App 早已不在——正是最需要还原的场景。
        if let uid = trust.pinnedUID {
            restoreSystemDNSAfterClientLoss(uid: uid)
        }
...
/// §2b.1 加固：单条连接的收 / 发硬超时（秒）。
///
/// 助手是单线程串行 accept 循环，任何一条连接阻塞都会让整个助手停摆——包括 30 秒自愈。
/// socket 权限是 0666（App 是普通用户进程，必须能 connect），所以"谁能连"挡不住，
/// 只能靠"连上了也拖不住"。5 秒对本机 socket 上几百 KB 的配置绰绰有余。
let ioTimeoutSeconds: Double = 5

/// 给一条连接设置收发超时。失败不致命（退化回原有阻塞行为），不因此拒绝服务。
func applyIOTimeout(fd: Int32, seconds: Double) {
    var tv = timeval(
        tv_sec: __darwin_time_t(seconds),
        tv_usec: __darwin_suseconds_t((seconds - Double(Int(seconds))) * 1_000_000)
    )
...
func readAllConfig(fd: Int32) -> Data? {
    ...
    let deadline = monotonicSeconds() + ioTimeoutSeconds
    while true {
        // §2b.1 加固：configFD 是 App 经 SCM_RIGHTS 递来的**管道**读端，
        // socket 的 SO_RCVTIMEO 管不到它。写端一直不关就会在这里永久阻塞，
        // 与挂死 socket 等效。每次读前 poll，并用单调时钟卡总时限。
        let remaining = deadline - monotonicSeconds()
        if remaining <= 0 { return nil }
        var pfd = pollfd(fd: fd, events: Int16(POLLIN), revents: 0)
        let ready = poll(&pfd, 1, Int32(remaining * 1000))
        if ready == 0 { return nil }                       // 超时
        if ready < 0 {
            if errno == EINTR { continue }
            return nil
        }
...
func handleConnection(connfd: Int32) {
    ...
    case .startKernel:
        ...
        // S2：把 root 能读的 rule_set 锁进被钉死那个用户的 App 支持目录。
        // 解析不出目录时传 nil（退回结构校验）——纵深防御不该把可用性搭进去。
        let allowedRuleSetDirectory = trust.pinnedUID.flatMap { clientSupportDirectory(forUID: $0) }
        let validation = HelperConfigWhitelist.validate(
            configData,
            allowedRuleSetDirectory: allowedRuleSetDirectory
        )
        ...
        // 词法前缀判断挡不住"支持目录里放个指向 /etc/… 的符号链接"，这里再用 realpath 复核。
        // 文件不存在 → realpath 失败 → 放行：不存在的文件没有内容可泄露，
        // 而因此拒绝启动会让"规则集还没下载完"变成 TUN 起不来。
        if let allowedRuleSetDirectory {
            for path in validation.ruleSetPaths {
                guard let real = resolvedRealPath(path) else { continue }
                guard HelperConfigWhitelist.isPathContained(real, in: allowedRuleSetDirectory) else {
                    sendResponse(HelperResponse(
                        ok: false,
                        message: "config rejected: rule_set resolves outside allowed directory",
                        helperVersion: helperVersion
                    ), connfd: connfd)
                    return
                }
...
// MARK: - F1：App 异常退出后还原系统 DNS

/// 起一个子进程并收走它的 stdout。仅用于 root 侧执行 `networksetup`。
///
/// 不用 Foundation 的 `Process`：它内部靠 dispatch source 监控子进程终止，而本文件顶层
/// 在 Swift 6 下是 @MainActor 隔离的，dispatch 回调一碰顶层状态就 SIGTRAP
///（见下方 §2b.1 那段注释里记录的真机崩溃）。裸 posix_spawn + waitpid 没有这个问题。
///
/// **必须先读空管道再 waitpid**：反过来的话，子进程输出超过管道缓冲就会阻塞在写上、
/// 父进程阻塞在等它退出，互相等死。
func runCapturingOutput(_ path: String, _ arguments: [String]) -> (exitCode: Int32, output: String)? {
    var fds: [Int32] = [-1, -1]
    guard pipe(&fds) == 0 else { return nil }
    let readEnd = fds[0]
    let writeEnd = fds[1]
    // 读端置 CLOEXEC：子进程不该拿到它，否则我们永远等不到 EOF。
    _ = fcntl(readEnd, F_SETFD, FD_CLOEXEC)
    ...
    let spawned = cStrings.withUnsafeMutableBufferPointer { buffer in
    ...
    while true {
        let n = buf.withUnsafeMutableBufferPointer { ptr -> Int in
            read(readEnd, ptr.baseAddress!, ptr.count)
        }
        if n == 0 { break }
        if n < 0 {
            if errno == EINTR { continue }
        ...
        data.append(contentsOf: buf[0..<n])
        if data.count > 1_048_576 { break }   // networksetup 不该输出这么多，防跑飞
    }
    close(readEnd)
    ...
    return (Int32(code), String(decoding: data, as: UTF8.self))
}

/// 读取并校验指定 uid 的 DNS 还原快照。任一校验不过返回 nil（拒绝优先）。
///
/// 这个文件位于用户自己的支持目录、由用户进程写入，对 root 来说是**不可信输入**。
/// 三道门：`O_NOFOLLOW` 拒符号链接、必须是属主为该 uid 的普通文件、体积封顶。
/// 内容的合法性由 `HelperDNSRestore.restoreArguments` 再过一遍。
func homeDirectory(forUID uid: UInt32) -> String? {
    guard let pw = getpwuid(uid_t(uid)), let dirPointer = pw.pointee.pw_dir else { return nil }
    let home = String(cString: dirPointer)
    guard !home.isEmpty, home != "/" else { return nil }
    return home
}

/// 被钉死那个用户的 App 支持目录。S2 用它限制 root 能读的 rule_set 范围。
func clientSupportDirectory(forUID uid: UInt32) -> String? {
    homeDirectory(forUID: uid).map { $0 + "/Library/Application Support/kongshan" }
}

/// 解析符号链接后的真实路径。文件不存在时返回 nil。
func resolvedRealPath(_ path: String) -> String? {
    guard let resolved = realpath(path, nil) else { return nil }
    defer { free(resolved) }
    return String(cString: resolved)
}

func loadDNSRecoverySnapshot(uid: UInt32) -> HelperDNSRestore.Snapshot? {
    guard let home = homeDirectory(forUID: uid) else { return nil }
    let path = home + "/Library/Application Support/kongshan/dns-recovery.json"
    ...
    return try? JSONDecoder().decode(HelperDNSRestore.Snapshot.self, from: data)
}

/// App 已消失时，按快照把系统 DNS 还原回接管前的样子。
///
/// **为什么这件事必须由助手做**：DNS 是 App 用 `networksetup` 指到 TUN 自身地址
///（如 172.19.0.1）的，而还原动作也在 App 里。App 被 kill -9 / 崩溃后，助手 30 秒自愈
/// 会停掉内核——于是 TUN 没了、那个地址死了，**DNS 却还指着它**：整机域名解析全废，
/// 直到用户想起来重开 App 才被 `reconcileInactiveTakeovers` 修好。2026-09-16 的整网中断
/// 就是这个形状（那次的触发源不同，但"DNS 指着已死地址、只有重开才恢复"完全一致）。
///
/// **顺序：先还原 DNS，再停内核。** 反过来会多出一段"路由已撤、DNS 仍指死地址"的真空期。
/// 此刻 TUN 还在，把 DNS 指回真实解析器后查询照样通得出去。
func restoreSystemDNSAfterClientLoss(uid: UInt32) {
    guard let snapshot = loadDNSRecoverySnapshot(uid: uid) else { return }
    guard let listing = runCapturingOutput("/usr/sbin/networksetup", ["-listallnetworkservices"]),
          listing.exitCode == 0 else {
        FileHandle.standardError.write(Data("kongshan-helper: 取网络服务列表失败，跳过 DNS 还原\n".utf8))
        return
    }
    ...
    for arguments in commands {
        guard let result = runCapturingOutput("/usr/sbin/networksetup", arguments), result.exitCode == 0 else {
            FileHandle.standardError.write(
                Data("kongshan-helper: DNS 还原失败：\(arguments.joined(separator: " "))\n".utf8)
            )
            continue
        }
        restored += 1
    ...
    FileHandle.standardError.write(
        Data("kongshan-helper: App 已不在，已还原 \(restored)/\(commands.count) 个网络服务的 DNS\n".utf8)
    )
}
...
func checkClientLiveness() {
    ...
    if kill(clientPID, 0) != 0 {
        ...
        guard let trust = HelperSecurity.loadTrustConfig(),
              let singBox = singBoxURL(from: trust) else { return }
        // F1：**先**把系统 DNS 还原回接管前的样子，再停内核。
        // 停了内核 TUN 就没了，而 DNS 还指着 TUN 自身地址——那正是"整机解析全废、
        // 只有重开 App 才恢复"的成因。顺序不能反。
        if let uid = trust.pinnedUID {
            restoreSystemDNSAfterClientLoss(uid: uid)
        }
        let pid = state.clearKernelPID()
...
while HelperSignals.terminationRequested == 0 {
    ...
    if n > 0 && (pfd.revents & Int16(POLLIN)) != 0 {
        ...
        if connfd >= 0 {
            ...
            // §2b.1 加固：助手是**单线程串行** accept 循环，一条连接阻塞住就等于整个助手停摆
            //（连 30 秒自愈也不再跑）。socket 是 0666、任意本地账号都能 connect，
            // 只要连上不发数据就能把 root 助手永久挂死。给收发都设硬超时：
            // 超时后 read 返回 EAGAIN → readFully 返回 -1 → 帧解析失败 → 这条连接被拒并关闭。
            applyIOTimeout(fd: connfd, seconds: ioTimeoutSeconds)
