...
struct PolicyGroupsView: View {
    ...
    @State private var showsTestURLSheet = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showsTestURLSheet) { SpeedTestURLSheet() }
        .navigationTitle("代理")
        ...
        .toolbar {
            ToolbarItemGroup(placement: .primaryAction) {
                Picker("出站模式", selection: state.outboundModeBinding) {
                ...
                // 测速的「怎么测」与「测完做什么」都收进这一个菜单：此前它们是并排的
                // 三个无标签灰图标（测速 / 选最快 / 参数），彼此形状相近、一眼分不清是干什么的，
                // 正是用户 09-17 说「代理页有点拥挤」的那一处。
                // 「测速全部」仍是独立按钮：⌘R 挂在真 Button 上才可靠。
                Menu {
                    Button("测速并选最快") { state.startFastestTest(in: currentGroup.name) }
                        .disabled(state.testableNodes.isEmpty || state.isTestingAllDelays || !isSelectable)
                    Divider()
                    // 测速参数迁自 设置→网络·测速：想改「怎么测」的人，上一秒刚在这里按过测速、
                    // 觉得结果不对。放在「测」旁边比隔两页更符合第一直觉。
                    Picker("测速方式", selection: state.speedTestMethodBinding) {
                    ...
                    if state.speedTestMethod == .urlTest {
                        Divider()
                        Button("测试地址…") { showsTestURLSheet = true }
                    }
                } label: {
                    Label("测速选项", systemImage: "slider.horizontal.3")
                }
                .help("测速并选最快，以及怎么测：TCP 握手直连节点服务器，快且稳；URL 测速经当前代理请求测试地址，测真实链路但更慢")
            }
        }
    }

    private func subtitle(currentGroup: PolicyGroup, optionCount: Int, selectedName: String?) -> String {
        // 不重复策略名：左列那一行已经选中高亮、并在其副标题里写了当前节点。
        // 标题栏再写一遍只是把本就紧张的横向空间让给冗余信息。
        var parts = ["\(optionCount) 个成员"]
    ...
    private func optionRow(
        ...
        .padding(.vertical, 2)
        // 行内容限宽，不跟着窗口一路拉到底。1400pt 窗宽下右列有 1000 多点，
        // 节点名贴在左边、协议与延迟贴在右边，中间一千多像素全是空白——
        // 扫一屏要把每个名字和它自己的延迟横跨整块空白连起来，正是「排版拥挤/难读」的来源。
        // 限宽后行长回到可扫读的区间，延迟仍是右对齐的一列（选中高亮由 List 铺满整行，不受影响）。
