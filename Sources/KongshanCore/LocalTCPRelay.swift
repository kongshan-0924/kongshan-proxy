...
public enum LocalTCPRelayError: Error, LocalizedError {
    ...
    /// 指定端口已被别的程序占着（局域网共享只用用户指定的端口，不另选）。
    ...
    public var errorDescription: String? {
        ...
        case let .portInUse(port): "端口 \(port) 已被其他程序占用"
        }
    }
}
...
public protocol LocalTCPRelaying: AnyObject, Sendable {
    ...
    /// 另起一个监听全部接口的入口，供局域网设备使用。**只绑用户指定的端口**：
    /// 其他设备把它存进了代理设置，悄悄换端口等于让它们全部失联。占用时抛 `portInUse`。
...
public final class LocalTCPRelay: LocalTCPRelaying, @unchecked Sendable {
    ...
    /// 局域网入口只用用户指定的端口。
    ///
    /// 旧实现借用了内核端口的选择逻辑 `stableHighPort`：它只认 20000–49151（助手白名单给内核
    /// 本地端口定的范围），而这里允许 1024–49151、默认 7890——于是默认端口**从来没被用过**，
    /// 每次开共享都换一个随机端口，还报「7890 被占用」。2026-09-19 真机：设置 7890、实际监听 33588，
    /// 用户在别的设备上两个都试了都不通。
    ///
    /// 刚关掉的旧监听可能还没释放干净（`NWListener.cancel` 是异步的），被占用时短暂重试。
    public func startLANSharing(port: UInt16, policy: LANPeerPolicy) async throws -> UInt16 {
        if let existing = lock.withLock({ lanPort }) { return existing }
        guard (1024...49151).contains(Int(port)), let nwPort = NWEndpoint.Port(rawValue: port) else {
            throw LocalTCPRelayError.invalidPort(port)
        }
        ...
        for attempt in 0..<4 {
            ...
            do {
                return try await listenForLAN(on: nwPort, port: port)
            } catch LocalTCPRelayError.portInUse {
                continue
            }
        }
        throw LocalTCPRelayError.portInUse(port)
    }

    private func listenForLAN(on nwPort: NWEndpoint.Port, port: UInt16) async throws -> UInt16 {
            ...
            candidate.stateUpdateHandler = { [weak self, weak candidate] state in
                ...
                case let .failed(error):
                    ...
                    if case .posix(.EADDRINUSE) = error {
                        gate.fail(LocalTCPRelayError.portInUse(port))
                    } else {
                        gate.fail(LocalTCPRelayError.listenerFailed(error.localizedDescription))
                    }
                case .cancelled:
...
extension LocalTCPRelay {
    ...
    static func isPrivatePeer(_ endpoint: NWEndpoint) -> Bool {
        ...
        case let .ipv4(address):
            ...
            // **刻意不含 100.64.0.0/10（CGNAT）**。2026-09-17 审计时它一度被记成"功能缺口"
            //（手机热点场景拿不到私网地址），复核后判定保持拒绝：
            // 那是运营商级 NAT 的**共享**地址段，同运营商的其他用户可能就在同一个 /10 内，
            // 与 RFC1918 不同，它**不是信任边界**。而局域网共享入口是无鉴权的
            //（SharingView 已明示），放行等于"一连上运营商网络就可能成为对其他订户开放的代理"。
            // 真要支持热点场景，正确做法是加鉴权，而不是放宽这条基线。
            return false
