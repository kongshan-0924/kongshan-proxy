...
public actor SystemProxyManager {
    ...
    /// 逐个网络服务读回「优先级 + DNS + 代理端口」，供「网络自检与修复」展示。
    ///
    /// 复用本类的 `networksetup` 执行器（含瞬时错误重试），不另起一套。
    /// 读不到某个服务的某一项就按"未设置"处理——自检是只读展示，
    /// 不该因为一个服务查不到就整份失败。
    public func inspectServices() async throws -> [NetworkServiceState] {
        let orderOutput = try await execute(["-listnetworkserviceorder"]).stdout
        let ordered = NetworkStateParser.serviceOrder(from: orderOutput)
        var states: [NetworkServiceState] = []
        for (order, name) in ordered {
            let dns = (try? await execute(["-getdnsservers", name]).stdout)
                .map(NetworkStateParser.dnsServers(from:)) ?? []
            var port: Int?
            if let raw = try? await execute(["-getwebproxy", name]).stdout,
               let state = try? SystemProxyCommands.proxyState(from: raw), state.enabled {
                port = state.port
            }
            states.append(NetworkServiceState(order: order, name: name, dnsServers: dns, proxyPort: port))
        }
        return states
    }
