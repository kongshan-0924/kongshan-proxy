...
/// 内核里一个 selector 的状态。
public struct SelectorState: Equatable, Sendable {
...
public actor ClashAPIClient {
    ...
    /// 各 selector 当前选中的成员与全部可选成员（GET /proxies）。
    public func selectorStates() async throws -> [String: SelectorState] {
        let (data, _) = try await perform(request(path: ["proxies"]))
        guard let root = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              let proxies = root["proxies"] as? [String: Any] else {
            throw ClashAPIError.invalidPayload
        }
        var result: [String: SelectorState] = [:]
        for (name, value) in proxies {
            guard let proxy = value as? [String: Any],
                  (proxy["type"] as? String)?.lowercased() == "selector",
                  let now = proxy["now"] as? String else { continue }
            result[name] = SelectorState(now: now, all: proxy["all"] as? [String] ?? [])
        }
        return result
    }
