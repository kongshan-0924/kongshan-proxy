        ...
        if settings.useSubscriptionRules {
            for (index, rule) in subscriptionRules.enumerated() {
                ...
                case let .single(type):
                    hit = matches(type, value: rule.value, input: input) ? rule.value : nil
                case .ruleSet:
                    // 说明命中的是规则集里的哪一项：「规则集 ai：域名后缀 openai.com」。
                    hit = ruleSetContents[rule.value]?.firstMatch(input).map { "规则集 \(rule.value)：\($0)" }
                case .geoIP:
                    // 国家 IP 库归内核所有，本地无从得知——不猜，按未命中处理（与内置 geoip 一致）。
                    hit = nil
                }
...
public struct SubscriptionRule: Codable, Equatable, Hashable, Sendable, Identifiable {
    /// 规则的匹配方式。
    ///
    /// 订阅的 `rules:` 里不只有单条规则。不少机场（包括用户自己的「空山」「Support」订阅）
    /// 通篇只写 `RULE-SET,<规则集名>,<目标>`，匹配内容放在 `rule-providers` 指向的远程文件里。
    /// 2026-09-18 之前这里只认单条规则，RULE-SET / GEOIP / MATCH 一律丢弃——「空山」订阅
    /// 18 条规则里 16 条 RULE-SET + 1 条 GEOIP + 1 条 MATCH，于是被判为「没有自带规则」，
    /// AI / 流媒体 / 广告 / 直连名单全部失效，所有海外流量都走当前选的那个节点。
    public enum Kind: Codable, Equatable, Hashable, Sendable {
        /// 单条规则：DOMAIN-SUFFIX / DOMAIN / DOMAIN-KEYWORD / IP-CIDR / PROCESS-NAME。
        case single(CustomRuleType)
        /// 引用 `rule-providers` 里的规则集；`value` 是规则集名。
        case ruleSet
        /// `GEOIP,<国家码>`；`value` 是大写国家码。
        case geoIP
    }

    /// 身份沿用旧格式（单条规则仍是 `类型|值|目标`），去重与界面身份不因本次改动而变化。
    public var id: String { "\(kindKey)|\(value)|\(target)" }
    public var kind: Kind
    public var value: String
    /// DIRECT / REJECT / 策略组名。
    ...
    public var type: CustomRuleType? {
        if case let .single(type) = kind { type } else { nil }
    }

    /// 规则页「类型」列显示的名称。
    public var kindDisplayName: String {
        switch kind {
        case let .single(type): type.displayName
        case .ruleSet: "规则集"
        case .geoIP: "GEOIP"
        }
    }
    ...
    private var kindKey: String {
        switch kind {
        case let .single(type): type.rawValue
    ...
    public init(type: CustomRuleType, value: String, target: String) {
        self.init(kind: .single(type), value: value, target: target)
    }

    /// 解析一行 Clash 规则。不支持的类型返回 nil 并被跳过。
    ///
    /// `MATCH` 不在这里处理：它不是"一条规则"，而是兜底出口，
    /// 由 `ClashSubscriptionConverter.matchTarget(from:)` 单独取出。
    public static func parse(_ line: String) -> SubscriptionRule? {
        ...
        // 域名统一小写。**sing-box 的域名规则区分大小写**（2026-09-18 实测 1.13.21：规则写
        // `ExAmPlE.com` 时小写请求 `example.com` 直接放行），而浏览器与系统送来的域名总是小写——
        // 规则里留着大写就是一条永远匹配不上的死规则。
        let normalized = switch type {
        case .domain, .domainSuffix, .domainKeyword: value.lowercased()
        case .ipCIDR, .processName: value
        }
        return SubscriptionRule(type: type, value: normalized, target: target)
    }

    /// 解析 `MATCH,<目标>`，返回目标；不是 MATCH 行返回 nil。
    public static func parseMatch(_ line: String) -> String? {
        let parts = line.split(separator: ",", omittingEmptySubsequences: false)
            .map { $0.trimmingCharacters(in: .whitespaces) }
        guard parts.count >= 2, parts[0].uppercased() == "MATCH", !parts[1].isEmpty else { return nil }
        return parts[1]
    }
}
