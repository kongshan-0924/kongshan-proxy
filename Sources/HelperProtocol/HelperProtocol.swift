...
public enum HelperConstants {
    ...
    /// v4：**钉死调用方 uid**（`pinnedUID`）。v3 及更早只认签名/路径/cdhash，不认「是谁在连」——
    /// socket 是 0666、目录 0711 可穿越，同机任意本地账号（含非管理员）只要启动同一份
    /// `/Applications/kongshan.app` 就通过全部校验，可无密码 `startTun` 以 root 劫持整机路由与 DNS。
    /// 同时 v4 起助手会在 App 消失后按快照还原系统 DNS（需要 uid 才知道读谁的快照）。
    public static let trustConfigVersion: Int = 4
    /// root 内核允许监听的回环地址。
...
public struct HelperTrustConfig: Codable, Sendable, Equatable {
    ...
    /// 钉死安装者的 uid（`getuid()` 写入）。**身份校验的第四个维度**。
    ///
    /// 签名 / identifier / bundle 路径 / cdhash 回答的都是「跑的是不是那个 App」，
    /// 没有一个回答「是谁在跑它」。同机第二个本地账号启动同一份 `/Applications/kongshan.app`
    /// 时这四项**全部成立**，于是可以无密码调 `startTun`：root 内核 `auto_route` 接管整机
    /// 默认路由、fake-IP 劫持 DNS，把包括管理员在内所有用户的流量导到他选的出口；
    /// 或直接 `stopTun` 掐掉管理员的代理。钉 uid 是唯一能区分这一点的东西。
    ///
    /// nil 仅表示 v3 及更早的旧配置（`isCurrent` 会因此为 false，走重装）。
    public var pinnedUID: UInt32?
...
public struct HelperClientIdentity: Equatable, Sendable {
    ...
    /// 对端进程的有效 uid（取自同一个 audit_token，与签名信息同源、无竞态）。
    /// nil 表示未提取到 → 按拒绝处理。
...
public enum HelperTrustEvaluation {
    public static func isTrusted(
        ...
        // v4：钉 uid。上面四项都只回答「跑的是不是那个 App」，回答不了「是谁在跑」——
        // 同机另一个本地账号启动同一份 App 时它们全部成立。拒绝优先：
        // trust 里钉了就必须相等，且对端 uid 取不到（nil）一律拒。
        if let pinnedUID = trust.pinnedUID {
            guard let actual = identity.uid, actual == pinnedUID else { return false }
        }
        return true
    }
}
...
public enum HelperConfigWhitelist {
    ...
    public struct Result {
        ...
        /// 配置里引用到的本地 rule_set 文件路径。
        ///
        /// 交回给 helper 是为了让它再做一次 **realpath 复核**：这里只能做词法前缀判断，
        /// 挡不住"支持目录里放一个指向 /etc/… 的符号链接"。词法 + realpath 两道合起来才闭合。
        public let ruleSetPaths: [String]

        public init(ok: Bool, reason: String?, sanitizedData: Data?, ruleSetPaths: [String] = []) {
            self.ok = ok
            self.reason = reason
            self.sanitizedData = sanitizedData
            self.ruleSetPaths = ruleSetPaths
        }
    }

    /// 判断一个绝对路径是否落在允许目录之下（词法判断，不碰文件系统）。
    /// 拒绝相对路径与任何 `..` 段——`/a/b/../../etc/x` 这种前缀匹配得上但实际逃逸了。
    public static func isPathContained(_ path: String, in directory: String) -> Bool {
        guard path.hasPrefix("/"), !directory.isEmpty else { return false }
    ...
    ///
    /// - Parameter allowedRuleSetDirectory: 允许 root 读取 rule_set 的目录（调用方传入
    ///   被钉死那个用户的 App 支持目录）。传 nil 时退回只做结构校验的旧行为——
    ///   helper 解析不出家目录时不该因此拒绝启动 TUN，纵深防御不等于把可用性搭进去。
    public static func validate(_ data: Data, allowedRuleSetDirectory: String? = nil) -> Result {
        ...
        // S2 加固：限制 root 的**读**面。
        //
        // 原先 `route` 只做"是不是对象"的结构检查，于是 App 被攻破后可以让 root sing-box
        // 去读任意文件（`route.rule_set[].path`）。这里把本地 rule_set 锁进被钉死那个用户的
        // App 支持目录。
        //
        // **只管 `route.rule_set`**：`dns.servers[].path` 是 DoH 的 URL 路径（`/dns-query`），
        // `outbounds[].transport.path` 是 WebSocket 路径，都不是文件路径——一并禁掉会直接
        // 搞死 DoH 与 ws 传输。
        var ruleSetPaths: [String] = []
        if let route = root["route"] as? [String: Any], let ruleSets = route["rule_set"] {
            guard let entries = ruleSets as? [[String: Any]] else {
                return reject("route.rule_set must be an array of objects")
            }
            for entry in entries {
                let type = entry["type"] as? String
                if type == "local" {
                    guard let path = entry["path"] as? String, !path.isEmpty else {
                        return reject("local rule_set missing path")
                    }
                    if let allowed = allowedRuleSetDirectory {
                        guard isPathContained(path, in: allowed) else {
                            return reject("rule_set path outside allowed directory")
                        }
                    }
                    ruleSetPaths.append(path)
                } else if type == "remote" {
                    // 远端 rule_set 不落在本地读取面上；App 侧不生成，这里也不放行，
                    // 免得成为"让 root 去连任意地址"的新入口。
                    return reject("remote rule_set not allowed")
                } else {
                    return reject("rule_set type not allowed: \(type ?? "missing")")
                }
            }
        }
        ...
        return Result(ok: true, reason: nil, sanitizedData: sanitized, ruleSetPaths: ruleSetPaths)
    }
    ...
    private static func reject(_ reason: String) -> Result {
        Result(ok: false, reason: reason, sanitizedData: nil, ruleSetPaths: [])
    }
}

// MARK: - F1：App 异常退出后由助手还原系统 DNS

/// `networksetup -listallnetworkservices` 的输出解析。
public enum HelperNetworkServices {
    /// 全部服务名——**包含被禁用的**（前缀 `*`）。
    ///
    /// 与 App 侧 `SystemProxyCommands.enabledServices` 刻意不同：那里是"要去接管谁"，
    /// 只该动启用中的；这里是"要去还原谁"，被禁用的服务上照样可能留着指向已死 TUN 的
    /// DNS（2026-09-16 的整网中断就是 Thunderbolt Bridge 这类服务留的残留）。
    /// 禁用不等于不存在，`-setdnsservers` 对禁用服务同样生效。
    public static func allNames(from output: String) -> Set<String> {
        var names: Set<String> = []
        for rawLine in output.components(separatedBy: .newlines) {
            var line = rawLine.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !line.isEmpty, !line.hasPrefix("An asterisk (*)") else { continue }
...
/// 服务名必须逐字出现在实时服务列表里（杜绝任意字符串进 argv），
/// 每个 DNS 地址必须过 `inet_pton`（杜绝选项注入，如 `-setdnsservers` 后面塞 `-setairportpower`）。
/// 校验不过的整条服务跳过，而不是"尽力而为"地部分执行。
public enum HelperDNSRestore {
    /// App 侧 `DNSRecoverySnapshot` 的只读镜像。只声明还原需要的字段，
    /// `capturedAt` 等一律忽略（多余字段不影响 Decodable）。
    public struct Snapshot: Decodable, Sendable, Equatable {
        ...
        public init(version: Int?, services: [Service]) {
            self.version = version
            self.services = services
        }
    }

    /// 只认这个快照版本。App 改了格式而助手没跟上时宁可不动，也不要按旧理解去写系统设置。
    public static let supportedVersion = 1

    /// 严格的 IPv4 / IPv6 字面量校验。`inet_pton` 不接受前后空白、端口、CIDR 与主机名。
    public static func isValidIPLiteral(_ value: String) -> Bool {
    ...
    public static func restoreArguments(
        ...
        for service in snapshot.services {
            ...
            // 任一地址非法 → 整条跳过。部分执行会留下一半正确的 DNS，比不动更难排查。
            guard service.servers.allSatisfy(isValidIPLiteral) else { continue }
            commands.append(
                ["-setdnsservers", service.name] + (service.servers.isEmpty ? ["Empty"] : service.servers)
            )
        }
        return commands
    }
